    ...
    IDENTIFIER '(' ')' ';'
{
	...
	{ 
		string str_index = to_string(printf_counter);
		string s = "" + tab +".section	.rodata\n"+".LC" + str_index + ":\n" + tab+".string"+ tab + $3 + "\n"+ tab + ".text";
		lc.push_back(s);
		$$ = new funcall_astnode(new identifier_astnode("printf"));
		$$->printf_counter = printf_counter;
		printf_counter++;
		$$->lvalue = false;
...
postfix_expression: 
  ...
  | postfix_expression '.' IDENTIFIER {
		...
		$$->lvalue = $1->lvalue;
		$$->offset2 = identifier_entry->offset;

        $$->offset = identifier_entry->offset;
		$$->offset += $1->offset;
  }
